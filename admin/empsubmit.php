...
if(isset($data['user_id']) && ($data['user_id'] > 0)){
	...
	$query="INSERT INTO tbl_user(user_name, user_nickname, user_address, user_gender, user_email, user_password, user_status, user_dateadd, user_role) VALUES ('".$data['user']."','".$data['nickname']."','".$data['address']."','".$data['gender']."','".$data['email']."','".md5($data['password'])."',1,'".date('Y-m-d')."',2)";
	$result = $mysqli->query($query) or die($mysqli->error.__LINE__);
